...
function paint_file_open(){
	const $input = $(E("input")).attr({type: "file", accept: ".paint"})
		.on("change", ()=> {
...
function file_save_as(){
	deselect();
	save_canvas_as(canvas, `${file_name.replace(/\.(bmp|dib|a?png|gif|jpe?g|jpe|jfif|tiff?|webp|raw)$/, "")}.paint`, (saved_file_path, saved_file_name) => {
		saved = true;
		document_file_path = saved_file_path;
		file_name = saved_file_name;
		update_title();
...
function file_upload_as(){
	canvas.toBlob(blob => {
		sanity_check_blob(blob, () => {
			...
			var file = new File([blob], id, {type: "image/png", lastModified: Date.now()});
			...
			xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					window.location.href = '/variations?id='+this.responseText;
				}
			};
			xhttp.send(form);
		});
...
function save_canvas_as(canvas, fileName, savedCallbackUnreliable){
	...
	canvas.toBlob(blob => {
		sanity_check_blob(blob, () => {
			const file_saver = saveAs(blob, `${file_name.replace(/\.(bmp|dib|a?png|gif|jpe?g|jpe|jfif|tiff?|webp|raw)$/, "")}.paint`);
